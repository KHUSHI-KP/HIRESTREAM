...
export const voteJob = async (req, res) => {
  try {
    ...
    // Update webinarRequested to true
    const response = await db
      .update(jobs)
      .set({ webinarRequested: true })
      .where(eq(jobs.jobId, jobId))
      .returning();

    res.status(200).json({
      success: true,
      message: "Webinar request registered successfully",
      data: response[0],
    });
  } catch (error) {
...
export const updateWebinar = async (req, res) => {
  try {
    const { jobId } = req.params;
    const { webinarRequested, webinarDate, webinarLink } = req.body;
    ...
    const updatedJob = await db
      .update(jobs)
      .set({
        webinarRequested: webinarRequested ?? true, // default true if not passed
