...
const JobManagement = () => {
  ...
  async function fetchJob() {
    try {
      // 1. Fetch updated job details
      const jobRes = await fetch(`/api/jobs/${selectedJob.jobId}`);
      const jobData = await jobRes.json();
      setSelectedJob(jobData.data);  // <-- update with fresh job info

      // 2. Fetch applications as before
      const appRes = await fetch(`/api/jobs/${selectedJob.jobId}/applied`);
  ...
  (job) =>
    job.webinarRequested === true ||
    job.webinarRequested === "t" ||
    job.webinarRequested === 1
);
 ...
 {/* ✅ Webinar Section */}
        {selectedJob?.webinarRequested && (
          <div className="mt-4 p-3 border rounded bg-gray-50 dark:bg-gray-800">
            <h3 className="font-semibold mb-2">Webinar Details</h3>
            <p>
              <strong>Requested:</strong> Yes
            </p>
            {selectedJob.webinarDate && (
              <p>
                <strong>Date:</strong>{" "}
                {new Date(selectedJob.webinarDate).toLocaleString()}
              </p>
            )}
            {selectedJob.webinarLink && (
              <p>
                <strong>Link:</strong>{" "}
                <a
                  href={selectedJob.webinarLink}
                  target="_blank"
